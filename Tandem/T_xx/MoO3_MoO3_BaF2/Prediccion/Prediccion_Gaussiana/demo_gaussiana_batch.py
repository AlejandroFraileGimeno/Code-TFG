...
# --- Filtrar casos validos y construir targets en batch ---
print("Construyendo grid de targets...")
valid_idx = []   # (i_sigma, j_f0)
valid_f0  = []
valid_sig = []
...
all_targets = np.stack([make_target(f0, sig)
                        for f0, sig in zip(valid_f0, valid_sig)])  # (N_valid, N_FREQS)

# --- Prediccion inversa (batch) ---
print(f"\nPrediccion inversa batch ({N_valid} muestras)...")
params_norm = inv_model.predict(all_targets, batch_size=BATCH_SIZE, verbose=1)
all_params  = params_norm * (param_max - param_min) + param_min  # (N_valid, 4)
...
if EVAL_MODE == "forward":
    ...
    all_T_pred = None  # se calcularia con TMM si EVAL_MODE == "tmm"
