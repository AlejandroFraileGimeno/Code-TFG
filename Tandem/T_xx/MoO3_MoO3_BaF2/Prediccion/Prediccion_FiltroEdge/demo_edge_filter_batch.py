# -*- coding: utf-8 -*-
"""
Barrido de filtros de borde -- VERSION BATCH -- T_xx  MoO3/MoO3/BaF2
======================================================================
...
Parametros del filtro:
  ...
  T_left  : transmitancia zona izquierda (stopband del HP / passband del LP)
  T_right : transmitancia zona derecha   (passband del HP / stopband del LP)
...
MARGIN    = 450.0    # ventana eval = [f_mid - MARGIN, f_mid + MARGIN]
...
T_COMBOS_HP = [(0.0, 1.0), (0.025, 0.95), (0.05, 0.9)]   # paso-alto
...
# --- Construir todos los targets validos ---
...
for filter_name, combos, tc_offset in [
    ...
    for ci, (T_left, T_right) in enumerate(combos):
        tc = tc_offset + ci
        ax = axes[ci]
        grid = r2_grid[tc, :, :].T    # (N_FLANK, N_FMID) para imshow
        im = ax.imshow(
    ...
    fig.suptitle(f"Filtro de borde -- {label}  [MoO3/MoO3/BaF2]  EVAL={EVAL_MODE}", fontsize=12)
    fig.tight_layout()
    fname_out = f"heatmap_{filter_name}.png"
    fig.savefig(OUT_DIR / fname_out, dpi=200, bbox_inches="tight")
    print(f"Guardado: {fname_out}")

# --- Ejemplos buenos y malos por tipo de filtro ---
...
for filter_name in ("highpass",):
    ...
    plot_ejemplos(buenos, f"Mejores 6 -- filtro {label} [MoO3/MoO3/BaF2]", f"ejemplos_buenos_{filter_name}.png")
    plot_ejemplos(malos,  f"Peores 6  -- filtro {label} [MoO3/MoO3/BaF2]", f"ejemplos_malos_{filter_name}.png")
