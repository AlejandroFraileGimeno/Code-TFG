# -*- coding: utf-8 -*-
"""
Barrido de dips gaussianos -- diseno inverso T_xx  MoO3/V2O5/BaF2
=================================================================
Para cada combinacion (f0, sigma) construye el objetivo:

    T(f) = 1 - exp( -(f - f0)^2 / (2*sigma^2) )

La red inversa predice (th1, th2, d1, d2) y se verifica con el modelo
elegido (forward NN o TMM).
El ajuste se mide SOLO en la ventana [f0 - K*sigma, f0 + K*sigma].

Salidas:
  resultados/heatmap_r2.png        -- R2 en el espacio (f0, sigma)
  resultados/ejemplos_buenos.png   -- 6 mejores ajustes
  resultados/ejemplos_malos.png    -- 6 peores ajustes
"""
...
# ============================================================
N_TRAIN    = 20_000
K          = 3           # ventana de evaluacion: [f0 - K*sigma, f0 + K*sigma]
A          = 1.0         # profundidad del dip (fija)

F0_MIN     = 500.0
F0_MAX     = 1300.0
N_F0       = 250         # puntos en f0

SIGMA_MIN  = 20.0
SIGMA_MAX  = 150.0
N_SIGMA    = 50          # puntos en sigma

FREQ_MIN   = 400.0
FREQ_MAX   = 1400.0

# "forward" usa la red forward NN (rapido)
# "tmm"     usa el calculo exacto TMM (lento pero mas preciso)
EVAL_MODE  = "forward"
# ============================================================
...
def evaluate(th1, th2, d1, d2):
    if EVAL_MODE == "forward":
        return forward_nn(th1, th2, d1, d2)
    return tmm_spectrum(th1, th2, d1, d2)
...
ax.set_title(
    f"Calidad ajuste inverso -- dip gaussiano  [MoO3/V2O5/BaF2]\n"
...
plot_ejemplos(casos_buenos, "Mejores 6 -- dip gaussiano [MoO3/V2O5/BaF2] (eval: forward NN)", "ejemplos_buenos.png")
plot_ejemplos(casos_malos,  "Peores 6  -- dip gaussiano [MoO3/V2O5/BaF2] (eval: forward NN)", "ejemplos_malos.png")
