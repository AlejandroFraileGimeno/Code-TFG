# -*- coding: utf-8 -*-
"""
Evaluación numérica del surrogate forward T_xx — MoO3/V2O5/BaF2
=================================================================
Genera N estructuras aleatorias fuera del entrenamiento,
calcula T_xx con TMM y con la red forward, y reporta R², MAE, RMSE.
"""
...
# CONFIG
# ============================================================
NUM_SEEDS  = 3      # modelos del ensemble
N_EVAL     = 100    # estructuras a evaluar
D_MIN      = 200
D_MAX      = 1200
SEED_EVAL  = 99     # semilla distinta al entrenamiento
# ============================================================
...
for i in range(1, NUM_SEEDS + 1):
    m = tf.keras.models.load_model(MODELS_DIR / f"Model_{i}seed" / "forward.keras", compile=False)
...
for i in range(N_EVAL):
    ...
    print(f"[{i+1:3d}/{N_EVAL}]  th1={th1:5.1f}  th2={th2:5.1f}  "
          f"d1={d1:6.0f}  d2={d2:6.0f}  |  R²={r2_str}  MAE={mae:.5f}  RMSE={rmse:.5f}")

elapsed = time.time() - t_start
print(f"\n--- Resultados sobre {N_EVAL} estructuras ({NUM_SEEDS} modelos, {elapsed:.0f} s) ---")
print(f"R²   = {np.mean(r2_list):.4f}  ±  {np.std(r2_list):.4f}  "
      f"(sobre {len(r2_list)} con espectro no plano)")
