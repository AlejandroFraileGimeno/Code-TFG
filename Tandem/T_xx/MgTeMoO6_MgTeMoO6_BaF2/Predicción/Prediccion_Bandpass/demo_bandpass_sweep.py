# -*- coding: utf-8 -*-
"""
Barrido de pasa-bandas -- diseno inverso T_xx  MgTeMoO6/MgTeMoO6/BaF2
=======================================================================
6 parametros del filtro pasa-banda:
  ls  : inicio zona baja izquierda
  le  : fin zona baja izquierda  (= inicio banda de paso)
  rs  : inicio zona baja derecha (= fin banda de paso)
  re  : fin zona baja derecha
  T_low  in [0.0, 0.3]  : transmitancia en zonas bajas
  T_high in [0.7, 1.0]  : transmitancia en banda de paso  (fijado a 1.0)

Parametros de barrido:
  T_LOW_LIST      analogous to A_PCT_LIST  (nivel de la zona baja)
  OUTER_WIDTH_LIST analogous to FLANK_LIST (anchura de cada zona baja)
  FLANK           anchura sigmoide (fija)
  Grid 2D: le x rs  (bordes de la banda de paso)

Salidas:
  resultados/heatmap_Tlow<X>_<EVAL_MODE>.png   (uno por T_low)
...
OUTER_WIDTH_LIST = [10, 20, 50]       # anchura zona baja a cada lado (cm-1)
T_HIGH           = 1.0
FLANK            = 50.0                        # anchura sigmoide (cm-1)

MARGIN    = 100.0

LE_MIN  = 600.0    # borde izq de banda de paso
LE_MAX  = 800.0
N_LE    = 50

RS_MIN  = 800.0    # borde der de banda de paso
RS_MAX  = 1000.0
N_RS    = 50

MIN_PASS_WIDTH = 50.0    # anchura minima de la banda de paso
MIN_OUTER      = 10   # anchura minima de cada zona baja (ya garantizada por OUTER_WIDTH)
...
def make_target(le, rs, T_low, T_high, outer_width, flank):
    """
    Pasa-banda: T_low fuera de [le, rs], T_high dentro.
    ls = le - outer_width,  re = rs + outer_width.
    Flancos suavizados con sigmoide de anchura flank.
    """
    w = max(flank, FLANK_EPS)
    rise = _sig(FREQS, le, w)    # sube en le
    fall = _sig(FREQS, rs, w)    # sube en rs (= bajada del pasabanda)
...
total_combos = len(T_LOW_LIST) * len(OUTER_WIDTH_LIST) * N_LE * N_RS
done = 0
    ...
    for kw, outer_width in enumerate(OUTER_WIDTH_LIST):
        for i, le in enumerate(le_vals):
            for j, rs in enumerate(rs_vals):
                ...
                target           = make_target(le, rs, T_low, T_HIGH, outer_width, FLANK)
                ...
                if done % 50 == 0 or done == total_combos:
                    print(f"[{done:5d}/{total_combos}]  T_low={T_low:.2f}  OW={outer_width:.0f}  "
                          f"le={le:.0f}  rs={rs:.0f}  R2={r2:.3f}")
...
results_sorted = sorted(all_results,
                        key=lambda r: r['r2'] if not np.isnan(r['r2']) else -999,
                        reverse=True)
