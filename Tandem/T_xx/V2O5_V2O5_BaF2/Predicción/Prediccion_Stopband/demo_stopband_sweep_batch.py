# -*- coding: utf-8 -*-
"""
Barrido de stop-bands -- VERSION BATCH -- diseno inverso T_xx  V2O5/V2O5/BaF2
===============================================================================
...
INVERSE_DIR = ROOT_PATH / "Models" / "T_xx" / "V2O5_V2O5_BaF2" / f"Inverse_N{N_TRAIN}"
FORWARD_DIR = ROOT_PATH / "Models" / "T_xx" / "V2O5_V2O5_BaF2" / "Forward"
        ...
        for i, left in enumerate(left_vals):
            for j, right in enumerate(right_vals):
                ...
                if left - MARGIN < FREQ_MIN or right + MARGIN > FREQ_MAX:
                    continue
                valid_cases.append((ka, kf, i, j, a_pct, flank, left, right))

N_valid = len(valid_cases)
print(f"  {N_valid} casos validos de {len(A_PCT_LIST)*len(FLANK_LIST)*N_LEFT*N_RIGHT} totales")

all_targets = np.stack([make_target(left, right, a_pct, flank)
                        for (ka, kf, i, j, a_pct, flank, left, right) in valid_cases])
...
for k, (ka, kf, i, j, a_pct, flank, left, right) in enumerate(valid_cases):
...
for ka, a_pct in enumerate(A_PCT_LIST):
    ...
    fig.suptitle(
        f"Stop-band sweep -- T_outside={a_pct}%  T_inside=0  [V2O5/V2O5/BaF2]  EVAL={EVAL_MODE}",
        fontsize=12)
    fig.tight_layout()
...
plot_ejemplos(casos_buenos, "Mejores 6 stop-bands [V2O5/V2O5/BaF2] (eval: forward NN)", "ejemplos_buenos.png")
plot_ejemplos(casos_malos,  "Peores 6 stop-bands [V2O5/V2O5/BaF2] (eval: forward NN)",  "ejemplos_malos.png")
