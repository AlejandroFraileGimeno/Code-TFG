...
C1         = 1.0    # peso CD_norm
C2         = 0.1    # peso R_total
MAXITER    = 100    # generaciones DE
POPSIZE    = 10     # individuos por parámetro
TOL        = 1e-4   # tolerancia DE
WINDOW_CM  = 10.0   # ventana de búsqueda del pico (cm⁻¹)
...
print(f"Barrido: {n_total} targets  ({FREQ_MIN}–{FREQ_MAX} cm⁻¹, paso {FREQ_STEP} cm⁻¹)")
...
for idx_t, target_cm in enumerate(targets_cm):
    ...
    if fname.exists():
        print(f"[{idx_t+1:3d}/{n_total}]  {target_cm:.0f} cm⁻¹  → ya existe, saltando")
        continue
    ...
    print(
        f"[{idx_t+1:3d}/{n_total}]  {target_cm:.0f} cm⁻¹  "
        f"θ={theta_best:.1f}°  d1={d1_best:.0f}  d2={d2_best:.0f} nm  "
        f"FoM={fom_nn:.4f}  "
