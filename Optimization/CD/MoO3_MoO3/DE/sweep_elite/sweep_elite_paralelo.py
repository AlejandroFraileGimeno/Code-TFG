# -*- coding: utf-8 -*-
"""
Barrido DE elite PARALELO — MoO3/MoO3
Igual que sweep_elite.py pero usa ProcessPoolExecutor (N_WORKERS=8).
Cada proceso carga sus propios modelos TF una sola vez.

Checkpointing: si target_XXX.npz ya existe, lo salta.
Comparte la carpeta results/ con sweep_elite.py.
"""
...
sys.path.insert(0, str(ROOT_PATH / "Surrogates" / "CD" / "MoO3_MoO3"))
...
FREQ_STEP  = 2
SWEEP_MIN  = 625    # rango de targets (cm-1)
SWEEP_MAX  = 875
# ---------------------------------------------------------------------------
...
def _init_worker():
    ...
    for i in range(1, NUM_SEEDS + 1):
        ...
        _proc_scalers_cd.append(auxf.load_scalers(_CD_MODEL_DIR / f"Model_{i}seed" / "scalers.json"))
    ...
    for i in range(1, NUM_SEEDS + 1):
        ...
        _proc_scalers_rt.append(auxf.load_scalers(_RT_MODEL_DIR / f"Model_{i}seed" / "scalers.json"))
...
if __name__ == "__main__":
    targets_cm = np.arange(SWEEP_MIN, SWEEP_MAX + FREQ_STEP, FREQ_STEP)
    n_total    = len(targets_cm)
    pending    = [t for t in targets_cm
                  if not (RESULTS_DIR / f"target_{t:.0f}.npz").exists()]

    print(f"Barrido elite paralelo: {n_total} targets  ({SWEEP_MIN:.0f}-{SWEEP_MAX:.0f} cm-1, paso {FREQ_STEP} cm-1)")
    print(f"DE: MAXITER={MAXITER}  POPSIZE={POPSIZE}  TOL={TOL}")
    print(f"Pendientes: {len(pending)}  |  Workers: {N_WORKERS}")
    print(f"Resultados en: {RESULTS_DIR}\n")

    if not pending:
        print("Todo ya completado.")
        sys.exit(0)
    ...
    print(f"\nBarrido elite completado en {(time.time()-t_start)/60:.1f} min")
