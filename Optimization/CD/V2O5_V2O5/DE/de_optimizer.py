...
"""
===========================================================
Differential Evolution — CD optimizer (NN surrogate) — V2O5 / V2O5
===========================================================
FoM = C1 x CD_norm_peak + C2 x R_total_at_peak   (C1 >> C2)

Parametros libres : theta (deg), d1 (nm), d2 (nm)
Estructura        : Air / V2O5(d1, phi=theta) / V2O5(d2) / Au
...
# CONFIG
# ---------------------------------------------------------------------------
TARGET_WAVELENGTH_UM = None    # um  (longitud de onda objetivo)
WINDOW_CM            = 50.0    # cm-1 (ventana de busqueda del pico)
...
_CD_MODEL_DIR = ROOT_PATH / "Models" / "CD"      / "V2O5_V2O5"
_RT_MODEL_DIR = ROOT_PATH / "Models" / "R_total" / "V2O5_V2O5"
_DATABASE     = str(ROOT_PATH / "Datasets" / "CD" / "V2O5_V2O5")
...
# ---------------------------------------------------------------------------
print("=" * 60)
print("  Differential Evolution (surrogate NN) — V2O5/V2O5")
print("=" * 60)
...
for ax in (ax1, ax2):
    if TARGET_WAVELENGTH_UM is not None:
        ax.axvline(TARGET_WAVELENGTH_UM, color="#898781", ls="--", lw=1,
                   label=f"target  {TARGET_WAVELENGTH_UM} um")
    ax.legend()
...
fig.suptitle(
    f"DE optimo V2O5/V2O5 — th={theta_best:.1f} deg   d1={d1_best:.0f} nm   d2={d2_best:.0f} nm\n"
