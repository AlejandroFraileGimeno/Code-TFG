# -*- coding: utf-8 -*-
"""
Barrido DE paralelo — V2O5/MgTeMoO6
Igual que sweep.py pero usa ProcessPoolExecutor para correr N_WORKERS
targets simultáneamente. Cada proceso carga sus propios modelos TF una sola vez.
...
sys.path.insert(0, str(ROOT_PATH / "Surrogates" / "CD" / "V2O5_MgTeMoO6"))
...
N_WORKERS  = 8                            # procesos paralelos
...
_CD_MODEL_DIR = ROOT_PATH / "Models" / "CD"      / "V2O5_MgTeMoO6"
_RT_MODEL_DIR = ROOT_PATH / "Models" / "R_total" / "V2O5_MgTeMoO6"
_DATABASE     = str(ROOT_PATH / "Datasets" / "CD" / "V2O5_MgTeMoO6")
...
# ---------------------------------------------------------------------------
# Modelos del proceso worker (cargados una sola vez por proceso en _init_worker)
# ---------------------------------------------------------------------------
_proc_models_cd  = []
_proc_scalers_cd = []
_proc_models_rt  = []
...
def _init_worker():
    """Inicializador de cada proceso worker: carga los modelos TF una sola vez."""
    ...
    for i in range(1, NUM_SEEDS + 1):
        _proc_models_cd.append(_make_fast_predict(tf_models.load_model(
            _CD_MODEL_DIR / f"Model_{i}seed" / f"Model_{i}seed.h5", compile=False
        )))
        _proc_scalers_cd.append(auxf._load_scalers(_CD_MODEL_DIR / f"Model_{i}seed" / "scalers.json"))

    _proc_models_rt, _proc_scalers_rt = [], []
    for i in range(1, NUM_SEEDS + 1):
        _proc_models_rt.append(_make_fast_predict(tf_models.load_model(
            _RT_MODEL_DIR / f"Model_{i}seed" / f"Model_{i}seed.h5", compile=False
        )))
        _proc_scalers_rt.append(auxf._load_scalers(_RT_MODEL_DIR / f"Model_{i}seed" / "scalers.json"))


# ---------------------------------------------------------------------------
# Predicción (usa los modelos del proceso actual)
# ---------------------------------------------------------------------------
def _predict_ensemble(models_list, scalers_list, theta, d1, d2):
    ...
    for predict_fn, sc in zip(models_list, scalers_list):
        sx, sy      = sc
        params_norm = auxf._scale_X(params_batch, sx)
        out         = auxf._unscale_y(predict_fn(params_norm), sy)
...
# ---------------------------------------------------------------------------
# Main — el guard if __name__ == "__main__" es obligatorio en Windows
# para que los workers no relancen el barrido al importar este módulo
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    ...
    if not pending:
        print("Todo ya completado. Ejecuta plot_sweep.py para ver las gráficas.")
        sys.exit(0)
    ...
    with ProcessPoolExecutor(max_workers=N_WORKERS, initializer=_init_worker) as executor:
        ...
        for future in as_completed(futures):
            ...
            completed += 1
            avg_per_target = (time.time() - t_start) / max(1, completed - (n_total - len(pending)))
            eta_min        = avg_per_target * (n_total - completed) / 60

            if elapsed is None:
                print(f"[{completed:3d}/{n_total}]  {target_cm:.0f} cm-1  -> ya existía")
            else:
                print(
                    f"[{completed:3d}/{n_total}]  {target_cm:.0f} cm-1  "
                    f"({elapsed:.0f}s  ~{eta_min:.1f} min restantes)"
                )

    print(f"\nBarrido completado en {(time.time()-t_start)/60:.1f} min")
    print(f"Archivos en: {RESULTS_DIR}")
    print("Ejecuta plot_sweep.py para ver las gráficas.")
