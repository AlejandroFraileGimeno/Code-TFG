...
def train_models(directory, database, ntrain=8000, nvalidation=2000, resume_training=False):
    directory = str(Path(directory))
    database  = str(Path(database))
    Path(directory).mkdir(parents=True, exist_ok=True)

    nfeatures = 4
    xtr, xva, ytr, yva, scalers = auxf.prepare_data(ntrain, nvalidation, database, return_scalers=True)
