# -*- coding: utf-8 -*-
"""utils_nn_forward — MoO3 / V2O5 bilayer (copia de MoO3/MoO3 con V2O5 importado)."""
...
from generalized_transfer_matrix_method import (
    Air, SiO2, MoO3, V2O5, LayeredStructure, calculate_circular_dichroism_ref,
)
...
def prepare_data(ntrain, nvalidation, database, return_scalers=False, target_file="CD_spectra_norm.csv"):
    ...
    if ndata_t > n_structures:
        raise ValueError(f"The database is smaller than ntrain + nvalidation: {n_structures} < {ndata_t}.")
    ...
    scalers = {
        ...
        "cd_min": float(min_CD), "cd_max": float(max_CD),
...
def save_scalers(scalers, path_scalers):
    serializable = {k: v.tolist() if isinstance(v, np.ndarray) else v for k, v in scalers.items()}
...
def _get_normalization_arrays(database, bilayer=True):
    ...
    min_complete = [min_angles] * len(angles[1]) + [min_thickness] * len(thickness[1]) + [min_freq]
    max_complete = [max_angles] * len(angles[1]) + [max_thickness] * len(thickness[1]) + [max_freq]
    return {
