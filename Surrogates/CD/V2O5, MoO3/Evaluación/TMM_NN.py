"""
Prediction -- CD_norm surrogate vs TMM (V2O5 / MoO3)
"""
...
def run_inference(model_dir, database, num_seeds=5, N_pred=5, alpha=0):
    ...
    for i in range(N_pred):
        ...
        ax.scatter(lambda_mu, CD_pred, s=5, c="blue", label="CD NN")
        ax.scatter(lambda_mu, CD_true, s=5, c="red",  label="CD TMM")
        ax.set_xlabel(r"$\lambda$ ($\mu$m)")
        ax.set_ylabel("CD reflection (normalizado)")
        ax.set_title(f"V2O5/MoO3 -- d1={d1} nm  d2={d2} nm  theta={theta} deg")
        ax.legend()
        fig.tight_layout()
...
if __name__ == "__main__":
    run_inference(
        model_dir=str(ROOT_PATH / "Models" / "CD" / "V2O5_MoO3"),
