...
def train_cd(database, output_dir, ntrain=8000, nvalidation=2000, seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    cd        = np.loadtxt(database + "/CD_spectra_norm.csv", delimiter=",")

    n_data, n_freq = cd.shape
    theta = angles.flatten()
    d1    = thickness[:, 0]
    d2    = thickness[:, 1]
    freqs = np.linspace(400, 1100, n_freq)
    ...
    def rows(idx_arr):
        r = np.concatenate([np.arange(i * n_freq, (i+1) * n_freq) for i in idx_arr])
        return r
